...
def test_fitlins_cli_derivative_tree_parity(tmp_path):
    ...
    assert {delta.caveat_id for delta in result.deltas if delta.caveat_id} == set()
    assert not any(delta.gate.startswith("caveat-bypassed:") for delta in result.deltas)
