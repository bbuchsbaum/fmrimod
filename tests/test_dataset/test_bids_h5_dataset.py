...
def test_bids_h5_archive_spec_validates_and_is_frozen() -> None:
    spec = BidsH5ArchiveSpec(compression_mode="parcellated", compression=0)
    assert spec.archive_format == "bids_h5_study"

    with pytest.raises(FrozenInstanceError):
        spec.compression = 4
    with pytest.raises(ConfigError, match="compression must be between"):
        BidsH5ArchiveSpec(compression=10)
    with pytest.raises(ConfigError, match="compression_mode"):
        BidsH5ArchiveSpec(compression_mode="unknown")
