...
class _MatrixModel:
    def __init__(self, X: NDArray[np.float64], Y: NDArray[np.float64], source: Any = None) -> None:
        self._X = X
    ...
    def design_matrix(self) -> pd.DataFrame:
        if self._source is not None and hasattr(self._source, "design_matrix"):
            try:
                ...
                pass
        return pd.DataFrame(self._X, columns=[f"coef_{idx}" for idx in range(self._X.shape[1])])
