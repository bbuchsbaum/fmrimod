...
from .latent import LatentDataset as LatentDataset
from .latent import latent_dataset as _latent_dataset
...
# LatentDataset and latent_dataset have moved to fmrimod/dataset/latent.py
# (see the module-level class re-export above). The class definition lived
# here historically because the dataset/compat surface predated the typed
# dataset/* layout; the constructor remains importable from this module as
# a warning shim to preserve backward compatibility.
def latent_dataset(
    scores: ArrayLike,
    ...
    *,
    ...
    """Deprecated compatibility shim for :func:`fmrimod.dataset.latent_dataset`."""
    warnings.warn(
        "fmrimod.dataset.compat.latent_dataset() is deprecated; "
        "use fmrimod.dataset.latent_dataset() instead.",
        DeprecationWarning,
        stacklevel=2,
    )
    return _latent_dataset(
        scores,
        loadings=loadings,
        tr=tr,
