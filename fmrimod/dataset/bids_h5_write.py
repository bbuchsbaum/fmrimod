...
class BidsH5ArchiveSpec:
    """Validated BIDS-HDF5 v1 archive layout options."""

    compression_mode: CompressionMode = "parcellated"
    compression: int = 4
    space: str = "MNI152NLin2009cAsym"
    archive_format: str = "bids_h5_study"
    version: str = "1.0"
    pipeline: str = "fmrimod"

    def __post_init__(self) -> None:
        if self.archive_format != "bids_h5_study":
            raise ConfigError(
                "BIDS-HDF5 archive format must be 'bids_h5_study'",
                parameter="format",
                value=self.archive_format,
            )
        if not str(self.version).startswith("1."):
            raise ConfigError(
                "BIDS-HDF5 archive version must be a v1 schema",
                parameter="version",
                value=self.version,
            )
        if self.compression_mode not in {"parcellated", "latent"}:
            raise ConfigError(
                "compression_mode must be 'parcellated' or 'latent'",
                parameter="compression_mode",
                value=self.compression_mode,
            )
        if self.compression < 0 or self.compression > 9:
            raise ConfigError(
                "compression must be between 0 and 9",
                parameter="compression",
                value=self.compression,
            )
        if not str(self.space):
...
def compress_bids_study(
    ...
    with h5py.File(path, "w") as handle:
        ...
        handle.attrs["version"] = spec.version
        handle.attrs["compression_mode"] = spec.compression_mode
        handle.attrs["writer_version"] = "fmrimod"

        bids_group = handle.create_group("bids")
        _write_string_scalar(bids_group, "space", spec.space)
        _write_string_scalar(bids_group, "pipeline", spec.pipeline)
        ...
        for subject_id, dataset in entries:
            for run_index in range(dataset.n_runs):
                ...
                if first_n_features is None:
                    ...
                    spec.compression_mode == "parcellated"
                    and n_features != first_n_features
                ):
                    raise ValueError(
                ...
                if spec.compression_mode == "latent":
                    basis, loadings, offset = _pca_encode(data, n_components)
                    data_group.create_dataset(
                        "basis",
                        data=basis,
                        **_compression_kwargs(spec.compression),
                    )
                    data_group.create_dataset(
                        "loadings",
                        data=loadings,
        ...
        if spec.compression_mode == "parcellated":
...
__all__ = ["BidsH5ArchiveSpec", "compress_bids_study"]
