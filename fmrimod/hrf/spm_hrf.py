...
_SPM_LEGACY_PARAM_NAMES = ("p1", "p2", "a1")


def _spm_params(
    *, delay: float, undershoot: float, dispersion: float,
    u_dispersion: float, ratio: float,
) -> dict[str, float]:
    return {
        "delay": delay,
...
def _spm_legacy_params(p1: float, p2: float, a1: float) -> dict[str, float]:
...
class SPMG1_HRF(HRF):
    ...
    def __post_init__(self) -> None:
        self.params = _spm_params(
            delay=self.delay, undershoot=self.undershoot,
...
class SPMG1_HRF_Legacy(HRF):
    ...
    def __post_init__(self) -> None:
        self.params = _spm_legacy_params(self.p1, self.p2, self.a1)
        self.param_names = list(_SPM_LEGACY_PARAM_NAMES)
