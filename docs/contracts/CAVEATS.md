...
| `spm-realized-column-convolution-gap` | Tier A SPM parity probes (early 2026) | Pattern A SPM-canonical workflows | new work-request, scope TBD | Even with the aligned SPM canonical, fmrimod's realised SPM regressor correlates with Nilearn's at ~0.93 (kernel-level correlation is 0.9993). The remaining gap is the convolution scheme — fmrimod's sub-TR sampling/integration differs from Nilearn's 50× oversampled boxcar convolution. Exit: align fmrimod's HRF convolution path to Nilearn's (oversampling rate, event placement, resampling back to frame times) so Pattern A parity hits correlation > 0.9999 / max abs diff < 1e-4 on identical events. |
