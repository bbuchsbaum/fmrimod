...
| B1 | FitLins-style BIDS workflow | FitLins CLI | BIDS Stats Model translator + `fmrimod` fit | Translator v1 implemented in `fmrimod.bids.stats_model`; `benchmarks/parity/tier_b_fitlins_bids/` verifies the constrained FitLins-style JSON subset and runs a real local FitLins CLI derivative-tree comparison on a tiny BIDS/fMRIPrep fixture. Effect, t, and variance maps pass the declared voxel-wise gates under the explicit Nilearn-compatible AR(1) path. |
...
| C2 | Regression second level | `SecondLevelModel` | native `ols_voxelwise` group reducer | Implemented in `benchmarks/parity/tier_c_second_level/`; synthetic covariate effects and t maps match Nilearn without caveats. Native fmrimod group p-values remain two-sided; the benchmark compares Nilearn's signed one-sided p map as an explicitly named parity quantity, not as `p_coef:<term>`. |
