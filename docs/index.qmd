...
[Construct HRFs, design matrices and contrasts as explicit Python objects, fit first-level and group models, and examine the design at each stage.]{.lead}
...
<a href="tutorials/real-data-fiac.qmd" aria-label="Open the real FIAC analysis">
  <img
    ...
    alt="A centered two-by-four axial montage of the FIAC participant's brain showing the same-sentence minus different-sentence t statistic from one two-run model after 6 mm smoothing of the BOLD data."
    loading="eager">
</a>
```
:::
...
Fit both runs of a published FIAC experiment in one neuroim-backed dataset,
state named sentence and speaker effects, and restore the statistic to a
neuroim brain image. A separate checked-in receipt verifies the explicit
Nilearn-compatible equal-weight pooling path.
...
it is composable, IDE-introspectable, and visible before fitting.
:::
